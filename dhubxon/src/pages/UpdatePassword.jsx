...
function UpdatePassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isEmailValid(Pass1)) {
      Swal.fire(
        "Invalid Format",
        "Your password must be at least 8 characters long and include at least one digit, one lowercase letter, and one uppercase letter.",
        "error"
      );
      e.preventDefault();
    } else {
      ...
      try {
        ...
        if (userType === "freelancer") {
          ...
          response = await axios.post(
            "http://127.0.0.1:5000/freelancer/updatepassword",
            { email, password }
          );
          ...
          response = await axios.post(
            "http://127.0.0.1:5000/client/updatepassword",
            { email, password }
          );
          if (response.status === 200) {
            Swal.fire({
