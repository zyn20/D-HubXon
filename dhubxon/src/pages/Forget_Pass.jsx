...
console.log("i am in freelancer usertype")
    ...
    try {
        const response = await axios.post('http://127.0.0.1:5000/freelancer/forgetpassword', { email });
    ...
    try {
        const response = await axios.post('http://127.0.0.1:5000/client/forgetpassword', { email });
  ...
  <div class="bg-white lg:w-6/12 md:7/12 w-8/12 shadow-3xl rounded-xl">
    ...
    <form class="p-12 md:p-24">
      <div class="flex items-center text-lg mb-6 md:mb-8">
        ...
        <input type="email"  required id="username" class="bg-[#E8EAED] rounded pl-12 py-2 md:py-4 focus:outline-none w-full" placeholder="Enter Your Email" value={email} onChange={(e)=>{setEmail(e.target.value)}}/>
      </div>
      
      <button class="bg-[#7E22CE] font-medium p-2 md:p-4 text-white uppercase w-full rounded" onClick={handleSubmit}> VERIFY</button>
    </form>
  </div>
 </div>
    </div>
