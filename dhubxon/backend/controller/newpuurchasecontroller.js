...
const handlePurchase = async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      ...
      // Retrieve the Product by itemId to get the zipPath
      const product = await Products.findOne({ where: { id: itemId } });
      if (product && product.zipPath) {
        // Construct a URL to the zip file
        const zipFileURL = `http://127.0.0.1:5000${product.zipPath}`;
        ...
        console.log(`No product found for itemId: ${itemId}, or no zipPath available.`);
      }
    }
